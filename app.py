...
# ============================================================
# 🖤 黑嚕嚕－台股盤中雷達 V3.6.26
# V3.6.26：正式 Forward 精簡版｜移除已完成研究介面，保留 Gate D 正式訊號與 Google Drive Safe Ledger
# ============================================================

st.set_page_config(page_title='🖤 黑嚕嚕－台股盤中雷達', page_icon='🖤', layout='wide', initial_sidebar_state='expanded')

V3_6_13_LABEL = 'V3.6.26｜正式 Forward 精簡版 / Google Drive Safe Ledger'
...
with t1:
    st.caption('V3.6.26｜正式盤中雷達。黑嚕嚕技術100分維持鎖定模型；外資/投信僅作資訊標籤，不參與 Gate D 正式分數。')
...
st.divider()
st.info('🧹 V3.6.26 已移除舊版 Gate D / 容量最佳化研究介面；正式策略維持鎖定，歷史研究不再於日常 App 重跑。')
...
def _v36251_oauth_cfg():
    try:
        z = dict(st.secrets.get('google_drive_oauth', {}))
    except Exception:
        z = {}
    return {
        'client_id': str(z.get('client_id','')).strip(),
        'client_secret': str(z.get('client_secret','')).strip(),
        'refresh_token': str(z.get('refresh_token','')).strip(),
        'token_uri': str(z.get('token_uri','https://oauth2.googleapis.com/token')).strip(),
        'folder_name': str(z.get('folder_name',V36251_FOLDER_NAME)).strip() or V36251_FOLDER_NAME,
        'state_filename': str(z.get('state_filename',V36251_STATE_FILENAME)).strip() or V36251_STATE_FILENAME,
    }

def _v36251_oauth_configured():
    c = _v36251_oauth_cfg()
    return bool(c['client_id'] and c['client_secret'] and c['refresh_token'])

def _v36251_drive_service():
    c = _v36251_oauth_cfg()
    if not _v36251_oauth_configured():
        raise RuntimeError('Google Drive OAuth 尚未完成 Streamlit Secrets 設定')
    try:
        ...
        raise RuntimeError(
            '缺少 Google Drive OAuth 套件，請更新 requirements.txt。'
    ...
    creds = Credentials(
        ...
        scopes=[V36251_DRIVE_SCOPE],
    )
    creds.refresh(Request())
    return build('drive','v3',credentials=creds,cache_discovery=False), c

def _v36251_find(service, name, mime_type=None, parent_id=None):
    ...
    if not fs:
        return None
    fs = sorted(fs, key=lambda x:x.get('modifiedTime',''), reverse=True)
    return fs[0]

def _v36251_get_or_create_folder(service, cfg):
    mime='application/vnd.google-apps.folder'
    f=_v36251_find(service,cfg['folder_name'],mime_type=mime)
    if f:
        return f
    return service.files().create(
        body={'name':cfg['folder_name'],'mimeType':mime},
...
o1.metric('OAuth設定','已完成' if _v36251_oauth_configured() else '尚未完成')
...
if last251:
    if last251.get('ok'):
        st.success(str(last251.get('message','Drive 同步成功')))
    else:
        st.warning(str(last251.get('message','Drive 同步失敗')))

oo1,oo2,oo3=st.columns(3)
if oo1.button('🔌 測試 OAuth / Drive',key='v36251_test',use_container_width=True):
    ok,msg=_v36251_drive_test()
...
if oo3.button('♻️ 從 Drive 還原帳本',key='v36251_restore',use_container_width=True):
    ds,msg=_v36251_drive_download_state()
    if isinstance(ds,dict):
        st.session_state['v3624_state']=ds
        st.session_state['v36252_cloud_ready']=True
